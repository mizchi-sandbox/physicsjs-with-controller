...
/***/ function(module, exports, __webpack_require__) {
	...
	var $, KeyMap, Renderer, createWorld, template;
	...
	window.Physics = __webpack_require__(3);
	...
	createWorld = function() {
	  var edgeBounce, viewportBounds, world;
	  world = Physics({
	    integrator: 'verlet',
	    maxIPF: 16,
	    timestep: 1000.0 / 300
	  ...
	  world.add([Physics.behavior('body-impulse-response'), Physics.behavior('body-collision-detection'), Physics.behavior('sweep-prune'), edgeBounce]);
	  return world;
	};

	window.addEventListener('load', function() {
	  var ball, pentagon, query, rect, world;
	  window.renderer = React.render(Renderer({}), document.body);
	  world = createWorld();
	  window.world = world;
	  ...
	  world.add(rect);
	  pentagon = Physics.body('convex-polygon', {
	    x: 130,
	    y: 30,
	    vertices: [
	      {
	...
	window.addEventListener('keydown', function(e) {
	  switch (KeyMap[e.keyCode]) {
	    case 'left':
	    case 'a':
	      return inputBuffer.left = true;
	    case 'up':
	    case 'w':
	      return inputBuffer.up = true;
	    case 'right':
	    case 'd':
	      return inputBuffer.right = true;
	    case 'down':
	    case 's':
	      return inputBuffer.down = true;
	  }
	});
	...
	window.addEventListener('keyup', function(e) {
	  switch (KeyMap[e.keyCode]) {
	    case 'left':
	    case 'a':
	      return inputBuffer.left = false;
	    case 'up':
	    case 'w':
	      return inputBuffer.up = false;
	    case 'right':
	    case 'd':
	      return inputBuffer.right = false;
	    case 'down':
	    case 's':
	      return inputBuffer.down = false;
	  }
	});
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports= function (locals) {  var bodies = "bodies" in locals ? locals.bodies : jade_globals_bodies;
	  return function() {
	    var tags = [];
	    tags.push(React.createElement("svg", {
	      className: "content",
	      width: 640,
	      height: 650,
	      style: {
	      ...
	      if ("number" == typeof $$obj.length) for (var $index = 0, $$l = $$obj.length; $$l > $index; $index++) {
	        ...
	        var med = body.state.angular.pos;
	        tags.push(React.createElement.apply(React, [ "g", {
	          key: body.uid,
	          transform: "translate(" + x + "," + y + ") rotate(" + med + ")"
	        } ].concat(function() {
	          var tags = [];
	          if ("circle" === body.name) tags.push(React.createElement("circle", {
	            cx: 0,
	            cy: 0,
	            r: body.radius,
	            fill: "none",
	            stroke: "red"
	          })); else if ("rectangle" === body.name) tags.push(React.createElement("rect", {
	            width: body.width,
	            height: body.height,
	            x: -body.width / 2,
	            y: -body.height / 2,
	            fill: "gray"
	          })); else if ("convex-polygon" === body.name) {
	            var vs = body.geometry.vertices;
	            var points = vs.map(function(v) {
	              return v.x + "," + v.y;
	            }).join(" ");
	            tags.push(React.createElement("polygon", {
	              points: points,
	              fill: "gray"
	            }));
	          } else tags.push(React.createElement("circle", {
	            cx: 0,
	            cy: 0,
	            r: 3,
	            fill: "none",
